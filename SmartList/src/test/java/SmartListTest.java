...
public class SmartListTest {
    ...
    @Test
    public void getAllThreeTypesTest() {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            int end = i;
            int mid = (begin + end) / 2;
            assertEquals(begin, list.get(begin));
            assertEquals(end, list.get(end));
            assertEquals(mid, list.get(mid));
        }
    }
    ...
    private static Class<?> getListClass() throws ClassNotFoundException {
        return Class.forName("com.example.smartList.SmartList");
    }
}